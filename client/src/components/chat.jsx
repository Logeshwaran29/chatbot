...
const BotMessage = ({ text, setImages, setOpen}) => {
  ...
  const content = Object.values(text).map((value, index) => {
    if (Array.isArray(value)) {  
      return (
        <div className="tt" key={index}>
          <div className="img-container" onClick={() => handleClick(value)}>
            <img src={value[0]} alt=""/>
            <p>Click here for more images☝🏻</p>
          </div>
        </div>
      );
    } else if (typeof value === 'string') {
      const [label, link] = value.split(': ');

      if (link && link.startsWith("https")) {
        return (
          <div className='tt' key={index}>
            <p>{label}: <a href={link} target="_blank" rel="noopener noreferrer">{link}</a></p>
          </div>
        );
      } else {
        return (
...
const Chat = () => {
  ...
  const sendMsg = async() => {
    ...
    if (cInput.trim() !== '') {
      ...
      await axios.post('http://192.168.1.113:5000/chat',{"query" : cInput.replace(/\./g, ' ').trim()})
      .then(response =>{
        setTimeout(() => {
          setChatMessages(prevMessages => [
            ...prevMessages,
          ...
          setloading(false);
          if(i === 10){
            setSuggestion(prevArray => [...prevArray, "Want to see more colleges? 🎓"]);
            seti(0);
          }
        }, 2000);
      })
